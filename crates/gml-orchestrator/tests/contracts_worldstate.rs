...
fn known_name_identity_and_missing_entity_id() {
    ...
    assert!(refs.contains("\"label\":\"Лиза\""), "entity_refs missing label: {refs}");
    ...
    let rows: Vec<&Value> = q["results"]
        ...
        .collect();
    assert!(!rows.is_empty() && rows[0]["hash"].as_str().map(|s| !s.is_empty()).unwrap_or(false));
    ...
    assert!(errors0(&bad)["error"].as_str().unwrap().contains("entity_id is required"));
}
...
fn batch_session() -> Session {
    ...
    assert_eq!(ret["applied"].as_array().unwrap().len(), 8, "batch applied: {ret}");
    ...
    assert_eq!(anchor["aliases"], json!(["Тёрнвейл", "Тёрнвейле", "Turnvale", "turnvale"]));

    // Stored-record shape checks (Python `hasattr(world, "state_records")` branch).
    let recs = &s.world.state_records;
    assert!(recs.iter().any(|r| r.text == "На площади закрыли ворота."
        && r.kind == "fact"
        && r.scope == "public"
        && r.location_id == "turnvale_square"
        && r.location_name == "Площадь Тёрнвейля"
        && r.region_id == "turnvale"
        && r.region_name == "Тёрнвейль"
        && r.scene_id == "turnvale_square_gate"
        && r.importance == "clue"
        && r.aliases.iter().any(|a| a == "Тёрнвейле")));
    assert!(recs.iter().any(|r| r.text == "GM_SECRET_SENTINEL прячется под сценой."
        && r.kind == "fact"
        && r.scope == "gm"));
    assert!(recs.iter().any(|r| r.text == "NPC_PRIVATE_SENTINEL хранить молчание."
        && r.kind == "npc_memory"
        && r.owner == "borin"
        && r.location_id == "secret_cellar"
        && r.aliases.iter().any(|a| a == "PRIVATE_ALIAS_SENTINEL")));
    assert!(recs.iter().any(|r| r.text == "SHARED_RUMOR_SENTINEL сказала Лиза только игроку."
        && r.kind == "rumor"
        && r.scope == "participants"
        && r.owner == "lysa"
        && r.subject == "player"));
    assert!(recs.iter().any(|r| r.text == "ENTITY_FACT_SENTINEL Борин утверждает, что Лизе 24."
        && r.kind == "rumor"
        && r.scope == "participants"
        && r.owner == "borin"
        && r.subject == "player"
        && r.entity_id == "lysa"
        && r.source_npc == "borin"));
    assert!(recs.iter().any(|r| r.kind == "goal" && r.text.contains("GOAL_SENTINEL") && r.owner == "borin"));
    s
}
...
fn big_batch_and_player_scope_isolation() {
    ...
    let shared = qry(&mut s, json!({"scope": "player", "query": "SHARED_RUMOR_SENTINEL"}));
    assert!(to_str(&shared).contains("SHARED_RUMOR_SENTINEL"));
    assert!(shared["results"].as_array().unwrap().iter().any(|r| {
        r.get("id").and_then(Value::as_str).map(|s| !s.is_empty()).unwrap_or(false)
    ...
    let entity_q = qry(&mut s, json!({"scope": "player", "query": "ENTITY_FACT_SENTINEL lysa borin"}));
...
fn player_place_query_and_gm_scope() {
    ...
    let place = qry(&mut s, json!({"scope": "player", "query": "что было в Тёрнвейле"}));
    ...
    assert!(place_row["hash"].as_str().map(|s| !s.is_empty()).unwrap_or(false));

    let gm = qry(&mut s, json!({"scope": "gm", "query": "GM_SECRET_SENTINEL"}));
...
fn participants_merge_then_not_added() {
    ...
    let pl = qry(&mut s, json!({"scope": "player", "query": "MULTI_PARTICIPANT_SENTINEL"}));
    assert!(to_str(&pl).contains("MULTI_PARTICIPANT_SENTINEL"));
    let mareth = qry(&mut s, json!({"scope": "npc", "npc_id": "mareth", "query": "MULTI_PARTICIPANT_SENTINEL"}));
    assert!(to_str(&mareth).contains("MULTI_PARTICIPANT_SENTINEL"));
    let lysa = qry(&mut s, json!({"scope": "npc", "npc_id": "lysa", "query": "MULTI_PARTICIPANT_SENTINEL"}));
    ...
    // Now lysa can see it.
    let lysa_after = qry(&mut s, json!({"scope": "npc", "npc_id": "lysa", "query": "MULTI_PARTICIPANT_SENTINEL"}));
...
fn targetless_participants_array() {
    ...
    assert_eq!(add["applied"][0].get("target").and_then(Value::as_str).unwrap_or(""), "");
    assert_eq!(applied0(&add)["participants"], json!(["player", "mareth"]));

    let pl = qry(&mut s, json!({"scope": "player", "query": "TARGETLESS_PARTICIPANTS_SENTINEL"}));
    assert!(to_str(&pl).contains("TARGETLESS_PARTICIPANTS_SENTINEL"));
    let mareth = qry(&mut s, json!({"scope": "npc", "npc_id": "mareth", "query": "TARGETLESS_PARTICIPANTS_SENTINEL"}));
...
fn query_cache_dedup_and_new_rows() {
    ...
    let first = qry(&mut s, json!({"scope": "gm", "query": "QUERY_CACHE_SENTINEL"}));
    assert_eq!(first["status"], "known");
    assert!(to_str(&first).contains("QUERY_CACHE_SENTINEL первая"));

    let repeat = qry(&mut s, json!({"scope": "gm", "query": "QUERY_CACHE_SENTINEL первая улика"}));
    assert_eq!(repeat["status"], "already_delivered");
    assert!(repeat["already_delivered"].as_i64().unwrap() >= 1);
    assert!(repeat["results"].as_array().map(|a| a.is_empty()).unwrap_or(true));
    assert!(repeat["text"].as_str().unwrap().contains("already delivered"));
    ...
    let new = qry(&mut s, json!({"scope": "gm", "query": "QUERY_CACHE_SENTINEL"}));
...
fn query_cache_resets_after_compaction() {
    ...
    let _ = qry(&mut s, json!({"scope": "gm", "query": "QUERY_CACHE_SENTINEL"}));
    ...
    let after = qry(&mut s, json!({"scope": "gm", "query": "QUERY_CACHE_SENTINEL"}));
...
fn query_pagination_with_limit() {
    ...
    let first = qry(&mut s, json!({"scope": "gm", "query": "QUERY_LIMIT_SENTINEL", "max_results": 1}));
    assert_eq!(first["results"].as_array().unwrap().len(), 1);
    let first_text = first["results"][0]["text"].as_str().unwrap().to_string();

    let second = qry(&mut s, json!({"scope": "gm", "query": "QUERY_LIMIT_SENTINEL", "max_results": 2}));
    ...
    assert!(second["results"][0]["text"].as_str().unwrap().contains("QUERY_LIMIT_SENTINEL"));
}
...
fn npc_scope_isolation_and_relationship_lookup() {
    ...
    let borin = qry(&mut s, json!({"scope": "npc", "npc_id": "borin", "query": "NPC_PRIVATE_SENTINEL GOAL_SENTINEL"}));
    ...
    let borin_shared = qry(&mut s, json!({"scope": "npc", "npc_id": "borin", "query": "SHARED_RUMOR_SENTINEL"}));
    assert!(!to_str(&borin_shared).contains("SHARED_RUMOR_SENTINEL"));
    // ...but IS visible in lysa scope (she is the owner/participant).
    let lysa_shared = qry(&mut s, json!({"scope": "npc", "npc_id": "lysa", "query": "SHARED_RUMOR_SENTINEL"}));
    assert!(to_str(&lysa_shared).contains("SHARED_RUMOR_SENTINEL"));

    // entity_id record (borin->player about lysa) is not visible to lysa.
    let lysa_entity = qry(&mut s, json!({"scope": "npc", "npc_id": "lysa", "query": "ENTITY_FACT_SENTINEL"}));
    assert!(!to_str(&lysa_entity).contains("ENTITY_FACT_SENTINEL"));

    let relation = qry(&mut s, json!({"scope": "npc", "npc_id": "borin", "query": "relationship borin player"}));
    ...
    assert!(rel_rows[0]["hash"].as_str().map(|s| !s.is_empty()).unwrap_or(false));
}
...
fn bad_batch_unknown_npc() {
    let mut s = batch_session();
    let bad = upd(&mut s, json!([{"type": "npc_memory", "text": "x", "npc_id": "ghost"}]));
    assert_eq!(bad["ok"], json!(false));
...
fn mutable_fact_add_update_delete() {
    ...
    let payload = s.world.fact("MUTABLE_FACT_SENTINEL", "player", None).as_tool_payload();
    ...
    let payload2 = s.world.fact("MUTABLE_FACT_SENTINEL", "player", None).as_tool_payload();
    assert!(!payload2["text"].as_str().unwrap().contains("MUTABLE_FACT_SENTINEL"));
}
...
fn relationship_lifecycle() {
    ...
    let q = qry(&mut s, json!({"scope": "npc", "npc_id": "borin", "query": "relationship borin player"}));
    ...
    let mut srq = StateRecordQuery::new("debug");
    srq.kinds = Some(&kinds);
    srq.owner = "borin";
    srq.subject = "player";
    let active: Vec<&gml_world::StateRecord> = s.world.state_records_for(&srq);
    assert_eq!(active.len(), 1);
    assert!(active[0].text.contains("доверяет игроку"));
    ...
    let active2: Vec<&gml_world::StateRecord> = s.world.state_records_for(&srq);
    assert!(!active2[0].text.contains("конфликт не должен"));
    ...
    assert_eq!(applied0(&del)["status"], "deleted");
    let gone = qry(&mut s, json!({"scope": "npc", "npc_id": "borin", "query": "RELATION_SENTINEL"}));
...
fn goal_replace_mode_deletes_prior_goals() {
    ...
    srq.owner = "borin";
    let active: Vec<&gml_world::StateRecord> = s.world.state_records_for(&srq);
    let texts: Vec<&str> = active.iter().map(|r| r.text.as_str()).collect();
    assert!(texts.iter().any(|t| t.contains("GOAL_TWO")), "GOAL_TWO active: {texts:?}");
    assert!(!texts.iter().any(|t| t.contains("GOAL_ONE")), "GOAL_ONE should be inactive: {texts:?}");
}
