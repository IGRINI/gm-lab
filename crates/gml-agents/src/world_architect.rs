...
static WORLD_ARCHITECT_SYSTEM: LazyLock<String> = LazyLock::new(|| {
    render_prompt(PromptId::WorldArchitectSystem, json!({}))
        .expect("embedded world architect system prompt must render")
});
                ...
                if let Value::Object(map) = &nested {
                    *working_draft = merge_draft(Some(working_draft.clone()), map);
                    ToolApplication {
                        args: nested,
                        changed: true,
                        result: render_prompt(PromptId::WorldArchitectDraftSuccess, json!({}))
                            .expect("embedded world architect draft-success prompt must render"),
                    }
                } else {
                    ToolApplication {
                        args: nested,
